...
| `fields.js` | Описание полей настройки для редактора StreamElements — русский (по умолчанию) |
| `fields.en.js` | То же самое, лейблы на английском |
| `fields.es.js` | То же самое, лейблы на испанском |
...
- **Аватары Twitch** — актуальные изображения профилей загружаются по логину для каждого участника чата и кэшируются на время работы виджета; если StreamElements уже передал URL аватара, используется он.
- **Фильтрация чата** — можно скрывать команды, начинающиеся с `!`, и все сообщения выбранных ботов. Имена ботов задаются через запятую в поле «Скрывать сообщения ботов».
...
5. Вкладка **Fields** → содержимое `fields.js` (или `fields.en.js` / `fields.es.js`, если удобнее панель настроек на английском/испанском — StreamElements не умеет сам определять язык браузера в этой вкладке, файл нужно выбрать вручную).
...
Все доступные параметры виджета (баннеры, шрифты, анимации, режимы прокрутки, позиционирование текста и фильтрация чата) описаны в `fields.js` и отображаются в виде удобной панели прямо в редакторе StreamElements — никаких правок кода не требуется для базовой настройки. Включите «Скрывать команды», чтобы убрать сообщения, начинающиеся с `!`; в поле «Скрывать сообщения ботов» перечислите логины через запятую (по умолчанию: `Nightbot, StreamElements`). Если нужно изменить логику поведения (например, добавить новый режим анимации или источник эмотов), правки вносятся в `javascript.js`.

**Язык панели настроек.** Вкладка Fields в редакторе StreamElements читает статический JSON и сама не выполняет код, поэтому автоматически подставить язык браузера стримера в неё нельзя. Вместо одного файла на русском в репозитории есть три одинаковых по структуре файла — `fields.js` (ru), `fields.en.js` (en), `fields.es.js` (es). Вставьте во вкладку Fields тот, что на нужном языке.
...
- **[2026-07-28]** Фильтрация команд перенесена в единую точку добавления сообщений. Теперь при включённой настройке «Скрывать команды» скрываются любые сообщения, текст которых после начальных пробелов начинается с `!`, включая встроенные preview-сообщения и тестового пользователя `!CommandUser`.
- **[2026-07-28]** Аватары пользователей теперь отображаются в обоих режимах: как в стандартном боксе сообщения, так и поверх пользовательской картинки `message_bg_image`. В режиме картинки аватар размещается рядом с никнеймом и по-прежнему управляется настройкой «Показывать аватары».
- **[2026-07-28]** Исправлено отображение аватаров Twitch: удалён нерабочий адрес `avatars.twitch.tv`, добавлено получение актуального CDN-адреса по логину с кэшированием и сохранён приоритет URL, который приходит от StreamElements. Исправление работает для всех участников чата без предварительного списка пользователей.
- **[2026-07-28]** Улучшена фильтрация сообщений: команды с `!` распознаются даже после начальных пробелов; добавлено поле `hidden_bots` для скрытия всех сообщений выбранных ботов без учёта регистра. Фильтр применяется как к обычным, так и к тестовым событиям StreamElements. По умолчанию указаны `Nightbot` и `StreamElements`.
